...
int main(void)
{
    ...
    PORTC->PCR[CS_NUM] = PORT_PCR_MUX(1U) | PORT_PCR_DSE_MASK; //cs
    PORTC->PCR[6] = PORT_PCR_MUX(2U); // MOSI
    PORTC->PCR[7] = PORT_PCR_MUX(2U); // MISO
    PORTC->PCR[DC_NUM] = PORT_PCR_MUX(1U) | PORT_PCR_DSE_MASK; // Enable DC pin
    PORTD->PCR[RS_NUM] = PORT_PCR_MUX(1U) | PORT_PCR_DSE_MASK;

    PTC->PDDR |= (1<<DC_NUM); // Set as out
    PTD->PDDR |= (1<<RS_NUM);
    PTC->PDDR |= (1<<CS_NUM);
    PTC->PDDR |= (1<<6);
    PTD->PDDR |= (1<<1);
    ...
    SPI0->BR = SPI_BR_SPPR(2) | SPI_BR_SPR(2); // Divide by (2*2)
